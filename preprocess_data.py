...
def prepair_data(path, marketcap, last_date, n,window_x,window_y, close=['null','interpolate','ffill'], vol=['null','interpolate','fill0'], periods= [10, 34, 89, 100, 200]):
    df = pd.read_csv(path)
...
# select tickers in top_n marketcap
    ...
    X = df_pivot.values.reshape(df_pivot.shape[0],2,-1)
    X[np.isnan(X)] = 0.0
    close = X[:,0, :]
