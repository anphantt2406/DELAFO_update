...
# 		self.ema = ema
	...
	@classmethod
	def from_existing_config(cls,path_data,path_marketcap,last_date, top_n, model_name,alpha = 0.5,timesteps_input=64,timesteps_output=19,close_fill='ffill',vol_fill='fill0',n_fold=10,batch_size=128,epochs=300,activation="sigmoid",l2=0.001,l2_1=0.01,l2_2= 0.01,units=32):
		X,y,tickers = prepair_data(path_data, path_marketcap, last_date,n=top_n,window_x=timesteps_input,window_y=timesteps_output, close=close_fill, vol=vol_fill, periods = [10,34])
		if model_name == "GRU":
...
# 	parser.add_argument('--periods', nargs="+", type=float, help='A list of numbers (separated by spaces)')
	...
	parser.add_argument('--close_fill', choices=['null','interpolate','ffill'], default='ffill',help='fill missing value for close price')
	parser.add_argument('--vol_fill', choices=['null','interpolate','fill0'], default='fill0',help='fill missing value for volume')  
